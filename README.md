...
| `POST /api/registrations` | public (see below)      | Register an attendee           |
...
- **`POST /api/registrations` is public**, per the current spec, so an
  unauthenticated caller passes `attendeeId` in the body — meaning anyone can
  register anyone. Require a session before this goes anywhere public.
